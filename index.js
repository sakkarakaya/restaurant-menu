...
function displayItems(menuList) {
    ...
    menuList.forEach(item => {
        ...
        <img src="${item.img}" alt="ham" class="photo">
        <div class="item-info">
            <header>
                <h4>${item.title}</h4>
...
function displayCategories() {
    ...
    menu.forEach(i => {
        ...
        if (categories.indexOf(i.category) === -1) {
            ...
            categoryContent += `<button type="button" class="filter-btn" data-id="${categories[x]}">${categories[x++]}</button>`
        }
    ...
    buttonElements.forEach((button) => {
        button.addEventListener("click", function (e) {
            const selectedCategory = e.target.getAttribute("data-id");
            const filteredMenu = menu.filter(
                (ite) => ite.category === selectedCategory);
